...
export function normalizeRow(row) {
  ...
  for (const [key, value] of Object.entries(row)) {
    ...
    let camelKey = key;
    if (k === 'busymode') camelKey = 'busyMode';
    else if (k === 'waittime') camelKey = 'waitTime';
    else if (k === 'customername') camelKey = 'customerName';
    else if (k === 'customerid') camelKey = 'customerId';
    else if (k === 'orderid') camelKey = 'orderId';
    else if (k === 'itemid') camelKey = 'itemId';
    else if (k === 'stallid') camelKey = 'stallId';
    else if (k === 'stallname') camelKey = 'stallName';
    else if (k === 'shopid') camelKey = 'shopId';
    else if (k === 'isveg') camelKey = 'isVeg';
    else if (k === 'paymentstatus') camelKey = 'paymentStatus';
    else if (k === 'paymentid') camelKey = 'paymentId';
    else if (k === 'providerpaymentid') camelKey = 'providerPaymentId';
    else if (k === 'paymentfailurereason') camelKey = 'paymentFailureReason';
    else if (k === 'paymentverifiedat') camelKey = 'paymentVerifiedAt';
    else if (k === 'idempotencykey') camelKey = 'idempotencyKey';
    else if (k === 'provideraccountid') camelKey = 'providerAccountId';
    else if (k === 'onboardingstatus') camelKey = 'onboardingStatus';
    else if (k === 'settlementstatus') camelKey = 'settlementStatus';
    else if (k === 'orderamountpaise') camelKey = 'orderAmountPaise';
    else if (k === 'shopamountpaise') camelKey = 'shopAmountPaise';
    else if (k === 'platformcommissionpaise') camelKey = 'platformCommissionPaise';
    else if (k === 'providertransferid') camelKey = 'providerTransferId';
    else if (k === 'providersettlementid') camelKey = 'providerSettlementId';
    else if (k === 'failurereason') camelKey = 'failureReason';
    else if (k === 'createdat') camelKey = 'createdAt';
    else if (k === 'updatedat') camelKey = 'updatedAt';
    else if (k === 'settledat') camelKey = 'settledAt';
    else if (k === 'providereventid') camelKey = 'providerEventId';
    else if (k === 'eventtype') camelKey = 'eventType';
    else if (k === 'payloadhash') camelKey = 'payloadHash';
    else if (k === 'processedat') camelKey = 'processedAt';
...
export const db = {
  ...
  async transaction(callback) {
    if (isPgActive && pool) {
      ...
      let backup;
      try {
        backup = JSON.parse(JSON.stringify(memStore));
        const result = await callback(db);
        return result;
      } catch (err) {
        if (backup) {
...
function executeMemRun(sql, params) {
  ...
  if (upper.startsWith('INSERT INTO ORDERS')) {
    const [id, customerName, customerId, type, payment, status, total, time, timestamp, paymentStatus, paymentId, providerPaymentId, paymentFailureReason, paymentVerifiedAt, idempotencyKey] = params;
    const newOrder = { 
...
function executeMemAll(sql, params) {
  ...
  if (upper.includes('FROM ORDERS WHERE PAYMENTID = ?')) {
    const [paymentId] = params;
    return memStore.orders.filter(o => o.paymentId === paymentId);
  }

  if (upper.includes('FROM ORDERS WHERE IDEMPOTENCYKEY = ?')) {
    const [idempotencyKey] = params;
    return memStore.orders.filter(o => o.idempotencyKey === idempotencyKey);
  }
...
export async function initDatabase() {
  ...
  await db.exec(`
    CREATE TABLE IF NOT EXISTS stalls (
      ...
      onboarding_status TEXT DEFAULT 'inactive',
      settlement_status TEXT DEFAULT 'disabled'
    );
  `);
  ...
  if (isPgActive) {
    ...
    await db.exec('ALTER TABLE orders ADD COLUMN IF NOT EXISTS paymentStatus TEXT;');
    await db.exec('ALTER TABLE orders ADD COLUMN IF NOT EXISTS paymentId TEXT;');
    await db.exec('ALTER TABLE orders ADD COLUMN IF NOT EXISTS providerPaymentId TEXT;');
    await db.exec('ALTER TABLE orders ADD COLUMN IF NOT EXISTS paymentFailureReason TEXT;');
    await db.exec('ALTER TABLE orders ADD COLUMN IF NOT EXISTS paymentVerifiedAt TEXT;');
    await db.exec('ALTER TABLE orders ADD COLUMN IF NOT EXISTS idempotencyKey TEXT;');
    await db.exec('ALTER TABLE orders ADD COLUMN IF NOT EXISTS payment_status TEXT;');
    await db.exec('ALTER TABLE orders ADD COLUMN IF NOT EXISTS payment_id TEXT;');
    await db.exec('ALTER TABLE orders ADD COLUMN IF NOT EXISTS provider_payment_id TEXT;');
    await db.exec('ALTER TABLE orders ADD COLUMN IF NOT EXISTS payment_failure_reason TEXT;');
    await db.exec('ALTER TABLE orders ADD COLUMN IF NOT EXISTS payment_verified_at TEXT;');
    await db.exec('ALTER TABLE orders ADD COLUMN IF NOT EXISTS idempotency_key TEXT;');
    await db.exec('ALTER TABLE stalls ADD COLUMN IF NOT EXISTS provider_account_id TEXT;');
    await db.exec('ALTER TABLE stalls ADD COLUMN IF NOT EXISTS onboarding_status TEXT DEFAULT \'inactive\';');
    await db.exec('ALTER TABLE stalls ADD COLUMN IF NOT EXISTS settlement_status TEXT DEFAULT \'disabled\';');
  } else if (isSqliteActive) {
    try { await db.run('ALTER TABLE orders ADD COLUMN paymentStatus TEXT'); } catch (_) {}
    try { await db.run('ALTER TABLE orders ADD COLUMN paymentId TEXT'); } catch (_) {}
    try { await db.run('ALTER TABLE orders ADD COLUMN providerPaymentId TEXT'); } catch (_) {}
    try { await db.run('ALTER TABLE orders ADD COLUMN paymentFailureReason TEXT'); } catch (_) {}
    try { await db.run('ALTER TABLE orders ADD COLUMN paymentVerifiedAt TEXT'); } catch (_) {}
    try { await db.run('ALTER TABLE orders ADD COLUMN idempotencyKey TEXT'); } catch (_) {}
    try { await db.run('ALTER TABLE orders ADD COLUMN payment_status TEXT DEFAULT \'pending\''); } catch (_) {}
    try { await db.run('ALTER TABLE orders ADD COLUMN payment_id TEXT'); } catch (_) {}
    try { await db.run('ALTER TABLE orders ADD COLUMN provider_payment_id TEXT'); } catch (_) {}
    try { await db.run('ALTER TABLE orders ADD COLUMN payment_failure_reason TEXT'); } catch (_) {}
    try { await db.run('ALTER TABLE orders ADD COLUMN payment_verified_at TEXT'); } catch (_) {}
    try { await db.run('ALTER TABLE orders ADD COLUMN idempotency_key TEXT'); } catch (_) {}
    try { await db.run('ALTER TABLE stalls ADD COLUMN provider_account_id TEXT'); } catch (_) {}
    try { await db.run('ALTER TABLE stalls ADD COLUMN onboarding_status TEXT DEFAULT \'inactive\''); } catch (_) {}
    try { await db.run('ALTER TABLE stalls ADD COLUMN settlement_status TEXT DEFAULT \'disabled\''); } catch (_) {}
  }
  ...
  await db.exec(`
    CREATE TABLE IF NOT EXISTS order_settlements (
      ...
      shop_amount_paise INTEGER NOT NULL,
  ...
  await db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_settlement_order_stall ON order_settlements (order_id, stall_id);');
  ...
  if (!stallCount || parseInt(stallCount.count, 10) === 0) {
    const stallsData = [
      { id: 'mangales-snacks', name: 'Southern Delight(Mangale Snacks)', category: 'The Perfect BITE, Every Time...', online: 1, busyMode: 0, waitTime: 0, rating: 4.6, logo: '🥘', img: 'https://images.unsplash.com/photo-1601050690117-94f5f6fa8bd7?auto=format&fit=crop&w=400&q=80', provider_account_id: 'acc_MANGALES_SNACKS_123', onboarding_status: 'active', settlement_status: 'enabled' },
      { id: 'tea-coffee', name: 'Tea & Coffee', category: 'Fresh brews, every cup', online: 1, busyMode: 0, waitTime: 0, rating: 4.3, logo: '☕', img: 'https://images.unsplash.com/photo-1509042239860-f550ce710b93?auto=format&fit=crop&w=400&q=80', provider_account_id: 'acc_TEA_COFFEE_123', onboarding_status: 'active', settlement_status: 'enabled' },
      { id: 'rohit-vadewale', name: 'Rohit Vadewale', category: 'Rohit Wadewale by Poonam Vijay & Co.', online: 1, busyMode: 0, waitTime: 0, rating: 4.1, logo: '🥟', img: 'https://images.unsplash.com/photo-1567337710282-00832b415979?auto=format&fit=crop&w=400&q=80', provider_account_id: 'acc_ROHIT_VADEWALE_123', onboarding_status: 'active', settlement_status: 'enabled' },
      { id: 'oodles-of-noodles', name: 'Oodles of Noodles', category: 'Self Service – Chinese & Indo-Chinese', online: 1, busyMode: 0, waitTime: 0, rating: 4.4, logo: '🍜', img: 'https://images.unsplash.com/photo-1569718212165-3a8278d5f624?auto=format&fit=crop&w=400&q=80', provider_account_id: 'acc_OODLES_NOODLES_123', onboarding_status: 'active', settlement_status: 'enabled' },
      { id: 'narayana', name: 'Narayana', category: 'South Indian Special', online: 1, busyMode: 0, waitTime: 0, rating: 4.5, logo: '🥞', img: 'https://images.unsplash.com/photo-1630383249896-424e482df921?auto=format&fit=crop&w=400&q=80', provider_account_id: 'acc_NARAYANA_123', onboarding_status: 'active', settlement_status: 'enabled' },
      { id: 'cool-cravings', name: 'Cool Cravings', category: 'Shakes, Mojitos & Cold Coffees', online: 1, busyMode: 0, waitTime: 0, rating: 4.3, logo: '🥤', img: 'https://images.unsplash.com/photo-1572490122747-3968b75cc699?auto=format&fit=crop&w=400&q=80', provider_account_id: 'acc_COOL_CRAVINGS_123', onboarding_status: 'active', settlement_status: 'enabled' }
    ];

    for (const s of stallsData) {
      await db.run(
        'INSERT INTO stalls (id, name, category, online, busyMode, waitTime, rating, img, logo, provider_account_id, onboarding_status, settlement_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [s.id, s.name, s.category, s.online, s.busyMode, s.waitTime, s.rating, s.img, s.logo, s.provider_account_id, s.onboarding_status, s.settlement_status]
      );
    }
  }

  // Backfill existing stalls with mock provider accounts
  await db.run(`
    UPDATE stalls SET 
      provider_account_id = 'acc_MANGALES_SNACKS_123', 
      onboarding_status = 'active', 
      settlement_status = 'enabled' 
    WHERE id = 'mangales-snacks' AND provider_account_id IS NULL
  `);
  await db.run(`
    UPDATE stalls SET 
      provider_account_id = 'acc_TEA_COFFEE_123', 
      onboarding_status = 'active', 
      settlement_status = 'enabled' 
    WHERE id = 'tea-coffee' AND provider_account_id IS NULL
  `);
  await db.run(`
    UPDATE stalls SET 
      provider_account_id = 'acc_ROHIT_VADEWALE_123', 
      onboarding_status = 'active', 
      settlement_status = 'enabled' 
    WHERE id = 'rohit-vadewale' AND provider_account_id IS NULL
  `);
  await db.run(`
    UPDATE stalls SET 
      provider_account_id = 'acc_OODLES_NOODLES_123', 
      onboarding_status = 'active', 
      settlement_status = 'enabled' 
    WHERE id = 'oodles-of-noodles' AND provider_account_id IS NULL
  `);
  await db.run(`
    UPDATE stalls SET 
      provider_account_id = 'acc_NARAYANA_123', 
      onboarding_status = 'active', 
      settlement_status = 'enabled' 
    WHERE id = 'narayana' AND provider_account_id IS NULL
  `);
  await db.run(`
    UPDATE stalls SET 
      provider_account_id = 'acc_COOL_CRAVINGS_123', 
      onboarding_status = 'active', 
      settlement_status = 'enabled' 
    WHERE id = 'cool-cravings' AND provider_account_id IS NULL
  `);
