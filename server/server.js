...
// In-Memory Real OTP Store & Transporter
const otpStore = new Map();

const smtpUser = process.env.SMTP_USER || process.env.EMAIL_USER || 'smartbite.sgu@gmail.com';
...
// Send Real 6-Digit OTP Email
app.post('/api/auth/send-otp', async (req, res) => {
  const { email } = req.body;
  if (!email) {
    return res.status(400).json({ success: false, message: 'Email address or mobile is required.' });
  }
  ...
  otpStore.set(cleanEmail, { code: realOtp, expiresAt });

  let emailSent = false;
  if (smtpPass) {
    try {
      await mailTransporter.sendMail({
        from: `"SGU Smart-Bite" <${smtpUser}>`,
        to: cleanEmail,
        subject: '🔑 Your SGU Smart-Bite 6-Digit Verification Code',
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; background: #0B132B; color: #FFFFFF; border-radius: 16px;">
            <h2 style="color: #E4002B; text-align: center;">SGU Smart-Bite Verification</h2>
            <p style="font-size: 14px; text-align: center; color: #CBD5E1;">Your 6-digit verification code is:</p>
            <div style="background: #1E293B; border: 2px solid #E4002B; padding: 16px; border-radius: 12px; text-align: center; font-size: 32px; font-weight: 800; letter-spacing: 8px; color: #FF1E43; margin: 20px 0;">
              ${realOtp}
            </div>
            <p style="font-size: 12px; color: #94A3B8; text-align: center;">This code will expire in 10 minutes. If you did not request this code, please ignore this message.</p>
          </div>
        `
      });
      emailSent = true;
  ...
  res.json({
    ...
    otpCode: realOtp,
    message: emailSent
      ? `Real OTP code sent to ${cleanEmail}!`
...
app.post('/api/auth/verify-otp', async (req, res) => {
  ...
  if (!email || !code) {
    return res.status(400).json({ success: false, message: 'Email/mobile and code are required.' });
  }
  ...
  if (!cached) {
    return res.status(400).json({ success: false, message: 'OTP expired or not requested. Please click Resend Code.' });
  }
  ...
  if (cached.code !== cleanCode) {
    return res.status(400).json({ success: false, message: 'Incorrect verification code. Please check your inbox and try again.' });
  }
